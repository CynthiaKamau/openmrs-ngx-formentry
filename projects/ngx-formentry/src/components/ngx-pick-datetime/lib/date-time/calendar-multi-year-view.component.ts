...
export class OwlMultiYearViewComponent<T> implements AfterContentInit {
  ...
  public previousEnabled(): boolean {
    ...
    return (
      !this.minDate || !this.isSameYearList(this._pickerMoment, this.minDate)
    );
  }
  ...
  public nextEnabled(): boolean {
    return (
      !this.maxDate || !this.isSameYearList(this._pickerMoment, this.maxDate)
    );
  }
  ...
  private isSameYearList(date1: T, date2: T): boolean {
    return (
      Math.floor(
        this.dateTimeAdapter.getYear(date1) / (YEARS_PER_ROW * YEAR_ROWS)
      ) ===
      Math.floor(
        this.dateTimeAdapter.getYear(date2) / (YEARS_PER_ROW * YEAR_ROWS)
      )
    );
  }
