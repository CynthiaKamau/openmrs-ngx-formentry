...
describe('Form Factory:', () => {
  ...
  it('should create leaf node', () => {
    ...
    let parentNode = new GroupNode(
      new QuestionBase({ key: 'key', label: 'label', type: 'type' }),
      null,
      null,
      null,
      'path.to.parent'
    ...
    const createdNode = factory.createNode(
      testQuestion,
      parentNode,
      parentControl
    );
    ...
    const testQuestion2 = new TextInputQuestion({
      type: 'text',
      key: 'key1',
      placeholder: 'text'
    });
    ...
    parentNode = new GroupNode(
      new QuestionBase({ key: 'key', label: 'label', type: 'type' })
    );
    const createdNode2 = factory.createNode(
      testQuestion2,
      parentNode,
      parentControl
    );
  ...
  it('should create a group node', () => {
    ...
    const groupQuestion2: QuestionGroup = new QuestionGroup({
      key: 'g2',
      type: 'group',
      label: '',
      questions: []
    });
    const testQuestion3 = new TextInputQuestion({
      type: 'text',
      key: 'key3',
      placeholder: 'text'
    });
    ...
    expect(createdNode.children[testQuestion.key] instanceof LeafNode).toBe(
      true
    );
    expect(
      createdNode.children[testQuestion.key].control instanceof AfeFormControl
    ).toBe(true);
    ...
    expect(createdNode.children[testQuestion2.key] instanceof LeafNode).toBe(
      true
    );
    expect(
      createdNode.children[testQuestion2.key].control instanceof AfeFormControl
    ).toBe(true);
    expect(createdNode.children[testQuestion2.key].question).toBe(
      testQuestion2
    );
    expect(createdNode.children[testQuestion2.key].path).toBe('g1.key2');

    expect(createdNode.children[groupQuestion2.key]).toBeTruthy();
    expect(createdNode.children[groupQuestion2.key] instanceof GroupNode).toBe(
      true
    );
    expect(
      createdNode.children[groupQuestion2.key].control instanceof AfeFormGroup
    ).toBe(true);
    expect(createdNode.children[groupQuestion2.key].question).toBe(
      groupQuestion2
    );
    ...
    expect(
      createdFormModel.get(testQuestion.key) instanceof AfeFormControl
    ).toBeTruthy();
    expect(createdFormModel.get(testQuestion2.key)).toBeTruthy();
    expect(
      createdFormModel.get(testQuestion2.key) instanceof AfeFormControl
    ).toBeTruthy();
    expect(createdFormModel.get(groupQuestion2.key)).toBeTruthy();
    expect(
      createdFormModel.get(groupQuestion2.key) instanceof AfeFormGroup
    ).toBeTruthy();
    expect(
      createdFormModel.get(groupQuestion2.key + '.' + testQuestion3.key)
    ).toBeTruthy();
    expect(
      createdFormModel.get(
        groupQuestion2.key + '.' + testQuestion3.key
      ) instanceof AfeFormControl
    ).toBeTruthy();
  });
  ...
  it('should create a group node used for presentation only e.g pages', () => {
    ...
    // To represent page
    const groupQuestion1: QuestionGroup = new QuestionGroup({
      key: 'g1',
      type: 'group',
      label: '',
      questions: []
    ...
    const parentControl = control.parent as
      | AfeFormArray
      | AfeFormGroup
      | AfeFormControl;
  ...
  it('should add and remove a child node to array node', () => {
    ...
    const childNode = factory.createArrayNodeChild(
      repeatingQuestion,
      createdNode
    );
    ...
    expect((childGroup.children['key2'] as LeafNode).question).toBe(
      testQuestion2
    );
    ...
    expect(model.get('0.key1')).toBe(
      (childNode.children['key1'] as LeafNode).control
    );
    expect(model.get('0.g1.key2')).toBe(
      (childGroup.children['key2'] as LeafNode).control
    );
  ...
  it('should create a form', () => {
    ...
    expect(
      createdForm.rootNode.children['Triage v0.01'] instanceof GroupNode
    ).toBeTruthy();
    ...
    expect(
      (repeating1.children[0] as GroupNode).children['otherDrugDetail']
    ).toBeTruthy();
    expect(
      (repeating1.children[0] as GroupNode).children['otherDrugDetail'].control
    ).toBeTruthy();

    expect(
      createdForm.rootNode.control.get('otherDrug.0.otherDrugDetail')
    ).toBe(
      (repeating1.children[0] as GroupNode).children['otherDrugDetail'].control
    );
